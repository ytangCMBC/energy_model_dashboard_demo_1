...
with st.sidebar:
    ...
    if trip_type == "In Service":
        ...
        route_opts = sorted(summary_df.loc[summary_df.get("Depot Code","")==depot, "route_short_name"].dropna().unique()) if depot else []
        ...
        direction_opts = sorted(summary_df.loc[dir_mask, "route_direction"].dropna().unique()) if route else []
        direction = st.selectbox("Direction", options=direction_opts) if direction_opts else None

        sid_opts = summary_df.loc[(summary_df.get("Depot Code","")==depot) &
                                  (summary_df["route_short_name"]==route) &
                                  (summary_df["route_direction"]==direction), "shape_id"].astype(str).unique() if (depot and route and direction) else []
        ...
        depot_col = "depot" if "depot" in summary_df.columns else ("depot_code" if "depot_code" in summary_df.columns else None)
        if not depot_col:
        ...
        meta_df = df_t.sort_values(["distance_km", "kwh_per_km_medium_duty", "kwh_per_km_heavy_duty"], ascending=[False, True, True]).head(1)
